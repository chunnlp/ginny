...
def run_interval_eval(
    ...
    dev_src_file = '{}.{}'.format(hparams.dev_prefix, hparams.src)
    dev_tgt_file = '{}.{}'.format(hparams.dev_prefix, hparams.tgt)
    ...
    if use_test_set and hparams.test_prefix:
        test_src_file = '{}.{}'.format(hparams.test_prefix, hparams.src)
        test_tgt_file = '{}.{}'.format(hparams.test_prefix, hparams.tgt)
...
def run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess,
    ...
    dev_ppl, test_ppl = run_interval_eval(
...
dev train(hparams, scope=None, target_session=''):
    ...
    if not hparams.attention:
        ...
        model_creator = num_model.AttentionModel
    ...
    log_file = os.path.join(out_dir, 'log_{}'.format(time.time()))
    log_f = tf.gfile.GFile(log_file, mode='a')
    utils.print_out('# log_file={}'.format(log_file), log_f)
    ...
    # Training loop
    step_time, checkpoint_loss, checkpoint_predict_count = 0., 0., 0.
    checkpoint_total_count = 0.
    speed, train_ppl = 0., 0.
    start_train_time = time.time()

    utils.print_out('# Start step {}, lr {}, {}'.format(
        global_step, loaded_train_model.learning_rate.eval(session=train_sess),
        time.ctime()), log_f)

    # Initialize all of the iterators
    skip_count = hparams.batch_size * hparams.epoch_step
    utils.print_out('# Init train iterator, skipping {} elements'.format(skip_count))
    train_sess.run(
    ...
    while global_step < num_train_steps:
        ...
        try:
            ...
            utils.print_out(
                '# Finish an epoch, step {}. Perform external evaluation'.format(global_step))
            ...
            train_sess.run(
                train_model.iterator.initializer,
                free_dict={train_model.skip_count_placeholder: 0})
            continue
    ...
    utils.print_out(
        '# Final, step {} lr {} '
        'step-time {:.2f} wps {:.2f}K ppl {:.2f}, {}, {}'.format(
            global_step, loaded_train_model.learning_rate.eval(session=train_sess),
    ...
    for metric in hparams.metrics:
        ...
        utils.print_out('# Best {}, step {} '
                        'step-time {:.2f} wps {:.2f}K, {}, {}'.format(
