...
def check_tensorflow_version():
    ...
    if tf.__version__ < min_tf_version:
        raise EnvironmentError('Tensorflow version must >= {}'.format(min_tf_version))

def safe_exp(value):
    # Exponentiation with catching of overflow errror
    try:
        ans = math.exp(value)
...
def print_hparams(hparams, skip_patterns=None):
    ...
    for key in sorted(values.keys()):
        if not skip_patterns or all(
            [skip_pattern not in key for skip pattern in skip_patterns]):
...
def load_hparams(model_dir):
    ...
    if tf.gfile.Exists(hparams_file):
        print_out('# Loading hparams from {}'.format(hparams_file))
...
def maybe_parse_standard_hparams(hparams, hparams_path):
    ...
    if tf.gfile.Exists(hparams_path):
        print_out('# Loading standard hparams from {}'.format(hparams_path))
...
def debug_tensor(s, msg=None, summarize=10):
    # Print the shape and value of a tensor at test time
    if not msg:
        msg = s.name
