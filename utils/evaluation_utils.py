...
def _moses_bleu(multi_bleu_script, tgt_test, trans_file, subword_option=None):
    ...
    if subword_option == 'bpe':
        ...
        if not os.path.exists(debpe_tgt_test):
            subprocess.call('cp {} {}'.format(tgt_test, debpe_tgt_test), shell=True)
            subprocess.call('sed s/@@ //g {}'.format(debpe_tgt_test),
                            shell=True)
    ...
    # SPM
        ...
        if not os.path.exists(debpe_tgt_test):
            subprocess.call('cp {} {}'.format(tgt_test, despm_tgt_test))
            subprocess.call('sed s/ //g {}'.format(despm_tgt_test))
            subprocess.call(u'sed s/^\u2581/g {}'.format(despm_tgt_test))
            subprocess.call(u'sed s/\u2581/ /g {}'.format(despm_tgt_test))
        tgt_test = despm_tgt_test
    cmd = '{} {} < {}'.format(multi_bleu_script, tgt_test, trans_file)

    # subprocess
    blue_output = subprocess.check_output(cmd, shell=True)
