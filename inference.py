...
from . import model as ginny_model
from . import model_helper
from .utils import misc_utils as utils
from .utils import ginny_utils
...
def _decode_inference_indices(model, sess, output_infer,
                              ...
                              tgt_eos,
                              subword_option):
    # Decode only a specific set of sentences
    utils.print_out('  decoding to output {}, num sents {}'.format(
    ...
    with codecs.getwriter('utf-8')(
        ...
        for decode_id in inference_indices:
            ginny_outputs, infer_summary = model.decode(sess)

            # get text translation
            assert ginny_outputs.shape[0] == 1
            translation = ginny_utils.get_translation(
                ginny_outputs,
                sent_id=0,
                tgt_eos=tgt_eos,
            ...
            if infer_summary is not None:
                ...
                utils.print_out('  save attention image to {}*'.format(image_file))
...
def inference(ckpt,
    ...
    if not hparams.attention:
        model_creator = ginny_model.Model
    elif hparams.attention_architecture == 'standard':
        model_creator = attention_model.AttentionModel
    elif hparams.attention_architecture in ['gnmt', 'gnmt_v2']:
        model_creator = attention_model.GNMTModel
    else:
...
def single_worker_inference(infer_model,
                            ...
                            inference_output_file,
                            hparams):
    output_infer = infer_output_file
    ...
    with tf.Session(
        ...
        if hparams.inference_indices:
            ...
            ginny_utils.decode_and_evaluate(
                'infer',
                loaded_infer_model,
                sess,
                output_infer,
...
def multi_worker_inference(infer_model,
    ...
    output_infer = '{}_{}'.format(inference_output_file, jobid)
    output_infer_done = '{}_done_{}'.format(inference_output_file, jobid)
    ...
    with tf.Session(
        ...
        with codecs.getwriter('utf-8')(
            ...
            for worker_id in range(num_workers):
                worker_infer_done = '{}_done_{}'.format(inference_output_file, worker_id)
                while not tf.gfile.Exists(worker_infer_done):
                    utils.print_out('  waitting job {} to complete'.format(worker_id))
                    time.sleep(10)

                with codecs.getreader('utf-8'))(
                    tf.gfile.GFile(worker_infer_done, mode='rb')) as f:
                    for translation in f:
                        final_f.write('{}'.format(translation))
